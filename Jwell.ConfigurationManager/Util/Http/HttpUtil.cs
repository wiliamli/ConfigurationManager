...
namespace Jwell.ConfigurationManager.Util.Http
{
    public class HttpUtil : IDisposable
    {
        ...
        public HttpUtil(IJwellOptions options) => _options = options;
        ...
        public async Task<HttpResponse<T>> DoGetAsync<T>(string url, int timeout)
        {
            ...
            {
                throw new JwellConfigException("Could not complete get operation", ex);
            }
            finally
            {
                response?.Dispose();
            ...
            throw new JwellConfigStatusCodeException(response.StatusCode, $"Get operation failed for {url}");
        }
