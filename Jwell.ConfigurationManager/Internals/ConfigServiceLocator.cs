...
namespace Jwell.ConfigurationManager.Internals
{
    public class ConfigServiceLocator : IDisposable
    {
        ...
        public ConfigServiceLocator(HttpUtil httpUtil, IJwellOptions configUtil)
        {
            _httpUtil = httpUtil;
            _options = configUtil;
        ...
        public async Task<IList<ServiceDto>> GetConfigServices()
        {
            ...
            if (services.Count == 0)
                throw new JwellConfigException("No available config service");
        ...
        private async Task UpdateConfigServices(int times)
        {
            ...
            throw new JwellConfigException($"Get config services failed from {url}", exception);
        }
